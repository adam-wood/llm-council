...
class TestConversationCRUD:
    ...
    def test_create_conversation(self, temp_data_dir, test_user_id):
        ...
        with freeze_time("2024-01-01 12:00:00"):
            conv = storage.create_conversation(test_user_id, "test-conv-1")
    ...
    def test_get_conversation(self, temp_data_dir, test_user_id):
        ...
        storage.create_conversation(test_user_id, "test-conv-1")

        conv = storage.get_conversation(test_user_id, "test-conv-1")
    ...
    def test_get_nonexistent_conversation(self, temp_data_dir, test_user_id):
        ...
        conv = storage.get_conversation(test_user_id, "non-existent")
    ...
    def test_save_conversation(self, temp_data_dir, test_user_id):
        ...
        # Reload and verify
        loaded = storage.get_conversation(test_user_id, "test-conv-1")
    ...
    def test_list_conversations(self, temp_data_dir, test_user_id):
        ...
        with freeze_time("2024-01-01 10:00:00"):
            storage.create_conversation(test_user_id, "conv-1")

        with freeze_time("2024-01-01 12:00:00"):
            storage.create_conversation(test_user_id, "conv-2")

        with freeze_time("2024-01-01 11:00:00"):
            storage.create_conversation(test_user_id, "conv-3")
    ...
    def test_list_conversations_metadata_only(self, temp_data_dir, test_user_id):
        ...
        conv = storage.create_conversation(test_user_id, "test-conv-1")
        storage.add_user_message(test_user_id, "test-conv-1", "Test message")
    ...
    def test_delete_conversation(self, temp_data_dir, test_user_id):
        ...
        storage.create_conversation(test_user_id, "test-conv-1")
        assert storage.get_conversation(test_user_id, "test-conv-1") is not None

        # Delete it
        result = storage.delete_conversation(test_user_id, "test-conv-1")

        assert result is True
        assert storage.get_conversation(test_user_id, "test-conv-1") is None

    def test_delete_nonexistent_conversation(self, temp_data_dir, test_user_id):
        """Test deleting a conversation that doesn't exist."""
        result = storage.delete_conversation(test_user_id, "non-existent")
...
class TestMessageOperations:
    ...
    def test_add_user_message(self, temp_data_dir, test_user_id):
        ...
        storage.create_conversation(test_user_id, "test-conv-1")
        storage.add_user_message(test_user_id, "test-conv-1", "Hello, world!")
    ...
    def test_add_user_message_to_nonexistent_conversation(self, temp_data_dir, test_user_id):
        ...
        with pytest.raises(ValueError, match="Conversation .* not found"):
            storage.add_user_message(test_user_id, "non-existent", "Test")
    ...
    def test_add_assistant_message(self, temp_data_dir, test_user_id):
        ...
        storage.add_assistant_message(test_user_id, "test-conv-1", stage1, stage2, stage3)
    ...
    def test_add_assistant_message_to_nonexistent_conversation(self, temp_data_dir, test_user_id):
        ...
        with pytest.raises(ValueError, match="Conversation .* not found"):
            storage.add_assistant_message(test_user_id, "non-existent", [], [], {})

    def test_message_ordering(self, temp_data_dir, test_user_id):
        """Test that messages maintain insertion order."""
        storage.create_conversation(test_user_id, "test-conv-1")

        storage.add_user_message(test_user_id, "test-conv-1", "Message 1")
        storage.add_assistant_message(test_user_id, "test-conv-1", [], [], {})
        storage.add_user_message(test_user_id, "test-conv-1", "Message 2")
...
class TestTitleOperations:
    ...
    def test_update_conversation_title(self, temp_data_dir, test_user_id):
        ...
        storage.create_conversation(test_user_id, "test-conv-1")
        storage.update_conversation_title(test_user_id, "test-conv-1", "Custom Title")
    ...
    def test_update_title_of_nonexistent_conversation(self, temp_data_dir, test_user_id):
        ...
        with pytest.raises(ValueError, match="Conversation .* not found"):
            storage.update_conversation_title(test_user_id, "non-existent", "Title")
...
class TestEdgeCases:
    ...
    def test_unicode_in_messages(self, temp_data_dir, test_user_id):
        ...
        storage.create_conversation(test_user_id, "test-conv-1")
        storage.add_user_message(test_user_id, "test-conv-1", "Hello 世界 🌍 emoji")
    ...
    def test_large_message_content(self, temp_data_dir, test_user_id):
        ...
        storage.add_user_message(test_user_id, "test-conv-1", large_content)
    ...
    def test_user_isolation(self, temp_data_dir):
        ...
        storage.create_conversation(user1, "conv-1")
        storage.create_conversation(user2, "conv-1")
        ...
        assert storage.get_conversation(user1, "conv-1") is not None
        assert storage.get_conversation(user2, "conv-1") is not None
