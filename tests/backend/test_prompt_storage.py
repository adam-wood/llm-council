...
class TestPromptFallbackChain:
    ...
    def test_get_default_prompt_when_no_custom(self, temp_data_dir):
        """Test fallback to default when no custom prompts exist."""
        prompt = prompt_storage.get_prompt_for_model("test/model", "stage1")

        # Should return default prompt
        assert prompt["name"] == DEFAULT_PROMPTS["stage1"]["name"]
        assert "{user_query}" in prompt["template"]

    def test_get_model_specific_prompt(self, temp_data_dir):
        ...
        prompt_storage.save_custom_prompts(custom_prompts)
    ...
    def test_get_custom_default_prompt(self, temp_data_dir):
        ...
        prompt = prompt_storage.get_prompt_for_model("any/model", "stage1")

        assert prompt["name"] == "Custom Default Stage 1"
        assert prompt["template"] == "Custom default: {user_query}"

    def test_model_specific_overrides_default(self, temp_data_dir):
    ...
    def test_fallback_for_different_model(self, temp_data_dir):
...
class TestPromptCRUD:
    ...
    def test_update_default_prompt(self, temp_data_dir):
        ...
        result = prompt_storage.update_prompt("stage1", new_prompt)

        assert result["defaults"]["stage1"]["name"] == "Updated Stage 1"
        assert result["defaults"]["stage1"]["template"] == "Updated: {user_query}"

    def test_update_model_specific_prompt(self, temp_data_dir):
        ...
        result = prompt_storage.update_prompt("stage1", new_prompt, model="test/model")

        assert "test/model" in result["models"]
        assert result["models"]["test/model"]["stage1"]["name"] == "Model Specific"

    def test_reset_default_prompt(self, temp_data_dir):
        """Test resetting default prompt to system default."""
        # First, update default
        prompt_storage.update_prompt("stage1", {"template": "Custom"})

        # Then reset
        result = prompt_storage.reset_prompt("stage1")

        # Should return system default
        prompt = result["defaults"]["stage1"]
        assert prompt["name"] == DEFAULT_PROMPTS["stage1"]["name"]

    def test_reset_model_specific_prompt(self, temp_data_dir):
        """Test resetting model-specific prompt."""
        # Create model-specific prompt
        prompt_storage.update_prompt("stage1", {"template": "Custom"}, model="test/model")

        # Reset it
        result = prompt_storage.reset_prompt("stage1", model="test/model")
    ...
    def test_reset_all_prompts(self, temp_data_dir):
        """Test resetting all prompts to system defaults."""
        # Create various custom prompts
        prompt_storage.update_prompt("stage1", {"template": "Custom 1"})
        prompt_storage.update_prompt("stage2", {"template": "Custom 2"}, model="test/model")

        # Reset all
        result = prompt_storage.reset_all_prompts()

        # Should have system defaults
        assert result["defaults"]["stage1"]["name"] == DEFAULT_PROMPTS["stage1"]["name"]
        assert result["models"] == {}

    def test_get_all_model_prompts(self, temp_data_dir):
        """Test retrieving all prompts at once."""
        # Setup custom prompts
        prompt_storage.update_prompt("stage1", {"template": "Default 1"})
        prompt_storage.update_prompt("stage2", {"template": "Model 2"}, model="test/model")

        result = prompt_storage.get_all_model_prompts()
...
class TestDataPersistence:
    ...
    def test_prompts_saved_to_file(self, temp_data_dir):
        """Test that prompts are saved to JSON file."""
        prompt_storage.update_prompt("stage1", {"template": "Test"})

        prompts_file = Path(temp_data_dir) / "prompts.json"
    ...
    def test_load_custom_prompts(self, temp_data_dir):
        ...
        loaded = prompt_storage.load_custom_prompts()
    ...
    def test_load_corrupted_json(self, temp_data_dir, monkeypatch):
        ...
        monkeypatch.setattr("backend.prompt_storage.PROMPTS_FILE", prompts_file)

        loaded = prompt_storage.load_custom_prompts()

        assert loaded == {"defaults": {}, "models": {}}

    def test_legacy_format_migration(self, temp_data_dir, monkeypatch):
...
class TestPromptMerging:
    ...
    def test_partial_custom_default_merges_with_system(self, temp_data_dir):
        ...
        prompt = prompt_storage.get_active_prompts()["stage1"]
    ...
    def test_model_prompt_merges_with_defaults(self, temp_data_dir):
...
class TestEdgeCases:
    ...
    def test_update_nonexistent_stage(self, temp_data_dir):
        """Test updating a stage that doesn't exist in defaults."""
        # Custom stages are stored but result includes system defaults
        result = prompt_storage.update_prompt("custom_stage", {"template": "Custom"})
    ...
    def test_empty_model_identifier(self, temp_data_dir):
        ...
        result = prompt_storage.update_prompt("stage1", {"template": "Test"}, model="")

        # System continues to function
        assert "defaults" in result
        # Note: Empty string models may not persist depending on implementation

    def test_cleanup_empty_model_entries(self, temp_data_dir):
        """Test that empty model entries are cleaned up on reset."""
        # Create model with multiple stage overrides
        prompt_storage.update_prompt("stage1", {"template": "A"}, model="test/model")
        prompt_storage.update_prompt("stage2", {"template": "B"}, model="test/model")
        ...
        # Reset stage2
        result = prompt_storage.reset_prompt("stage2", model="test/model")

        # Model entry should be cleaned up
        assert "test/model" not in result["models"]

    def test_get_active_prompts_returns_all_stages(self, temp_data_dir):
        """Test that get_active_prompts returns all default stages."""
        prompts = prompt_storage.get_active_prompts()

        assert "stage1" in prompts
        assert "stage2" in prompts
        assert "stage3" in prompts

    def test_unicode_in_prompts(self, temp_data_dir):
        """Test handling of Unicode in prompt templates."""
        prompt_storage.update_prompt("stage1", {
    ...
    def test_very_long_prompt_template(self, temp_data_dir):
        """Test handling of very long prompt templates."""
        long_template = "A" * 10000 + " {user_query}"

        prompt_storage.update_prompt("stage1", {"template": long_template})
