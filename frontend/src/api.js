 ...
 * Uses factory pattern to inject authentication tokens.
 */

// Use relative URLs in production (same domain), localhost in dev
const API_BASE = import.meta.env.DEV ? 'http://localhost:8001' : '';

/**
 * Create an authenticated API client.
 * @param {function} getToken - Clerk's getToken function
 * @returns {object} API methods with auth
 */
export function createAuthenticatedApi(getToken) {
  ...
  return {
    ...
    async getAgent(agentId) {
      const response = await authFetch(`${API_BASE}/api/agents/${agentId}`);
      if (!response.ok) {
    ...
    async initializeDefaultAgents() {
      const response = await authFetch(`${API_BASE}/api/agents/initialize`, {
        method: 'POST',
      });
      if (!response.ok) {
     ...
     */
    async getChairman() {
      const response = await authFetch(`${API_BASE}/api/agents/chairman`);
      if (!response.ok) {
    ...
    async setChairman(agentId) {
      ...
      const response = await authFetch(`${API_BASE}/api/agents/chairman/${id}`, {
        method: 'PUT',
      });
      if (!response.ok) {
